...
class PersistentRenderer:
    ...
    def __init__(self, name: str, width: int = 960, height: int = 720):
        ...
        self._geometries: List[o3d.geometry.Geometry] = []  # Manual tracking of geometries
    ...
    def fit_view(self):
        ...
        if not self.vis or not self._view_control:
            return
        ...
        self._camera_params['zoom'] = 0.02 + (extent / 1000.0)  # Empirical adjustment
...
class SceneManager:
    ...
    def load_model(self, filepath: str) -> bool:
        ...
        try:
            ...
            self.model = o3d.geometry.TriangleMesh()
            self.model.vertices = o3d.utility.Vector3dVector(tm.vertices)
            self.model.triangles = o3d.utility.Vector3iVector(tm.faces)
    ...
    def get_geometries(self, trajectories: List[Trajectory], selected_trajectory: Optional[Trajectory], selected_waypoint: Optional[int]) -> List[o3d.geometry.Geometry]:
        ...
        for traj in trajectories:
            ...
            for wp in traj.waypoints:
                color = [1.0, 0.0, 0.0] if selected_trajectory == traj and selected_waypoint == wp.index else list(traj.color)
                ...
                rot_yaw = o3d.geometry.get_rotation_matrix_from_axis_angle(np.array([0,0,1]) * yaw_rad)
                rot_pitch = o3d.geometry.get_rotation_matrix_from_axis_angle(np.array([0,1,0]) * pitch_rad)
            ...
            try:
                ...
                if 'cameras' in data:
                    for i, cam in enumerate(data['cameras']):
                        pos = np.array(cam.get('position', [0,0,0])[:3])
                        yaw = cam.get('yaw', 0.0)
                        pitch = cam.get('pitch', 0.0)
                        waypoints.append(Waypoint(pos, yaw, pitch, i))
                elif 'waypoints' in data:
                    for i, wp_data in enumerate(data['waypoints']):
                        waypoints.append(Waypoint.from_dict(wp_data, i))
                else:
                    raise ValueError("No 'cameras' or 'waypoints' key found")
